...
HISTORICAL_TEAM_NAMES: dict[str, str] = {
    ...
    # Depuis la canonicalisation CHH→CHA, on n'a plus qu'un seul code pour
    # Charlotte. La franchise actuelle (depuis 2014) s'appelle "Hornets",
    # et la NBA a réattribué l'historique des Hornets 1988-2002 à CHA.
    "CHA": "Hornets de Charlotte",
...
# Abréviations équipe : certains datasets utilisent des codes legacy (PHL,
# GOS, SAN, UTH) ou un alias historique (CHH = "Charlotte Hornets" 1988-2002,
# dont la franchise a déménagé à NO ; la nouvelle "Charlotte Hornets" depuis
# 2014 est CHA — la NBA a *réattribué* l'historique à CHA en 2014). On
# canonicalise tout vers une seule abréviation par franchise actuelle pour
# éviter les doublons de catégories type `team_phl` + `team_phi`.
#
# Décisions :
#  - PHL → PHI    (76ers ; PHL = legacy ABA/CSV)
#  - GOS → GSW    (Warriors)
#  - SAN → SAS    (Spurs)
#  - UTH → UTA    (Jazz)
#  - CHH → CHA    (Hornets — réattribution officielle NBA 2014)
#
# On NE MERGE PAS :
#  - NJN/BKN, NOH/NOK/NOP, SEA/OKC, VAN/MEM : franchise *renommée/déménagée*,
#    identité distincte côté fan (Jason Kidd "Nets de New Jersey" ≠ Brooklyn).
TEAM_ABBR_CANONICAL = {
    "PHL": "PHI",
    "GOS": "GSW",
    "SAN": "SAS",
...
def _canon_team(abbr: str) -> str:
    """Renvoie l'abréviation canonique pour une franchise (déduplication)."""
    if not abbr:
        return abbr
    return TEAM_ABBR_CANONICAL.get(abbr, abbr)
...
def _aggregate_player_games(rows: list[dict]) -> dict[int, dict]:
    ...
    for row in rows:
        try:
            ...
            team_abbr = _canon_team(row["MATCHUP"].split(" ")[0])
...
def _aggregate_historical(rows: list[dict]) -> dict[int, dict]:
    ...
    for row in rows:
        try:
            ...
            team = _canon_team(row["TEAM_ABBREVIATION"])
...
def _merge_live_snapshots(
    ...
    for snap in snapshots:
        ...
        for abbr_raw, plist in (snap.get("rosters") or {}).items():
            if not year:
                break
            if not isinstance(plist, list):
                continue
            abbr = _canon_team(abbr_raw)
            for entry in plist:
        ...
        for pid_str, ts_list in (snap.get("player_team_seasons") or {}).items():
            ...
            for entry in (ts_list or []):
                if not isinstance(entry, list) or len(entry) != 2:
                    continue
                abbr_raw, year = entry[0], entry[1]
                if not abbr_raw or not isinstance(year, int):
                    continue
                abbr = _canon_team(abbr_raw)
